...
def estimate_entry_waiting_time():
	...
	entry_waiting_time, entry_waiting_time_days, current_entry_churn, entry_churn, entry_churn_time_days = calculate_wait_time(active_validators, beacon_entering, "entry")

	return entry_waiting_time, entry_waiting_time_days, beacon_entering, active_validators, current_entry_churn, entry_churn
...
def estimate_exit_waiting_time():
	...
	exit_waiting_time, exit_waiting_time_days, current_exit_churn, exit_churn, exit_churn_time_days = calculate_wait_time(active_validators, beacon_exiting, "exit")

	return exit_waiting_time, exit_waiting_time_days, beacon_exiting, current_exit_churn, exit_churn
...
def calculate_wait_time(active_validators, queue, type):
	...
	current_entry_churn = 8
	current_exit_churn = 14
	churn_time_days = 0
	churn_factor = 0
	
	for i, item in enumerate(scaling):
		if active_validators > scaling[i]:
			current_entry_churn = epoch_churn[i]
			current_exit_churn = epoch_churn[i]
			if current_entry_churn > max_churn:
				current_entry_churn = max_churn
		if (active_validators >= scaling[i]) and (active_validators < scaling[i+1]):
			...
			while queue_remaining > 0:
				...
				if epoch_churn_j > max_churn and type == "entry":
					epoch_churn_j = max_churn
					day_churn_j = max_daily_churn
	...
	current_churn = current_exit_churn
	if type == "entry":
		current_churn = current_entry_churn
	if type == "entry" and current_churn > max_churn:
		current_churn = current_entry_churn
...
def update_historical_data(current_entry_churn, current_exit_churn):
	with open('historical_data.json', 'r') as f:
		...
		todays_data =  {
			...
			"current_entry_churn":current_entry_churn,
			"current_exit_churn":current_exit_churn,
...
def generate_html(entry_waiting_time, beacon_entering, exit_waiting_time, beacon_exiting, active_validators, current_entry_churn, current_exit_churn, amount_eth_staked, percent_eth_staked, staking_apr, historical_data):
	html_content = f"""<!DOCTYPE html>
		...
		<body>
			...
			<div class="container">
				{header(current_time)}
				{overview(entry_waiting_time, beacon_entering, exit_waiting_time, beacon_exiting, current_entry_churn, current_exit_churn, active_validators, amount_eth_staked, percent_eth_staked, staking_apr)}
				{faq}
...
entry_waiting_time, entry_waiting_time_days, beacon_entering, active_validators, current_entry_churn, entry_churn = estimate_entry_waiting_time()
exit_waiting_time, exit_waiting_time_days, beacon_exiting, current_exit_churn, exit_churn = estimate_exit_waiting_time()
eth_supply, amount_eth_staked, percent_eth_staked, staking_apr = network_data()
historical_data = update_historical_data(current_entry_churn, current_exit_churn)

generate_html(entry_waiting_time, beacon_entering, exit_waiting_time, beacon_exiting, active_validators, current_entry_churn, current_exit_churn, amount_eth_staked, percent_eth_staked, staking_apr, historical_data)
